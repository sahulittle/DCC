...
const CertificationContent = () => {
    ...
    const calculateSavingsPercentage = (faceValue, memberPrice) => Math.round(((faceValue - memberPrice) / faceValue) * 100);
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <div className="bg-gradient-to-br from-primary/10 to-secondary/10 py-20">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center max-w-3xl mx-auto">
                        <h1 className="font-heading text-5xl md:text-6xl font-bold text-foreground mb-6">Redeemable Certificates</h1>
            ...
            <div className="py-20">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-12">
                        <h2 className="font-heading text-4xl font-bold text-foreground mb-4">How Certificates Work</h2>
                        <p className="text-xl text-muted-foreground">Three simple steps to instant savings</p>
                    </div>
                    <div className="grid md:grid-cols-3 gap-8 mb-16">
                        {[
                            { step: '1', title: 'Purchase', desc: 'Buy certificates at discounted member prices. Pay less than face value.' },
                            { step: '2', title: 'Receive', desc: 'Get your certificate instantly in your member dashboard.' },
                            { step: '3', title: 'Redeem', desc: 'Use the full face value at participating businesses. Keep the savings!' }
                        ].map((item) => (
                            <div key={item.step} className="text-center">
                                <div className="w-20 h-20 bg-primary text-primary-foreground rounded-full flex items-center justify-center mx-auto mb-6 text-3xl font-bold">{item.step}</div>
            ...
            <div className="bg-gradient-to-br from-primary/5 to-secondary/5 py-20">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-12">
                        <h2 className="font-heading text-4xl font-bold text-foreground mb-4">Available Certificates</h2>
                        <p className="text-xl text-muted-foreground">Purchase now and save instantly</p>
                    </div>
                                    ...
                                    <div className="bg-gradient-to-br from-primary/10 to-secondary/10 p-6 text-center">
                                        <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                                            {cert.businesses?.logo_url ? (
                                                <AppImage src={cert.businesses.logo_url} alt={cert.businesses.name || 'Business'} className="w-16 h-16 object-contain rounded-full" />
                                            ) : (
                                                <Icon name="TicketIcon" size={40} className="text-primary" />
                                            )}
                                        </div>
                                        <h3 className="font-heading text-2xl font-bold text-foreground mb-2">{cert.businesses?.name}</h3>
                                        <p className="text-muted-foreground">{cert.title}</p>
                                    </div>
                                    <div className="p-6">
                                        {cert.description && <p className="text-muted-foreground text-sm mb-6">{cert.description}</p>}
                                            ...
                                            <div className="border-t border-border pt-3">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="text-right">
                                                        <span className="text-xl font-bold text-success">${calculateSavings(cert.face_value, cert.member_price)}</span>
                                                        <span className="text-sm text-success ml-2">({calculateSavingsPercentage(cert.face_value, cert.member_price)}% off)</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <span className="text-sm text-muted-foreground">{cert.quantity_sold} sold</span>
                                        </div>
            ...
            <div className="py-20">
                <div className="max-w-4xl mx-auto px-6">
                    <div className="bg-primary text-primary-foreground rounded-2xl p-12 text-center">
                        <h2 className="font-heading text-4xl font-bold mb-6">Your Membership Pays for Itself</h2>
